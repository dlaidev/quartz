...
| Historical server logs | Recorded range |
| --- | --- |
| Prompt-throughput bursts | 4.18k–5.05k tok/s |
| Warm generation throughput | 21.7–22.0 tok/s |

![Dual-R9700 serving profile](/images/qwen35a3b_r9700_serving_profile.svg)
The request table describes one successful post-warm streamed request. Server counters cover reporting intervals; they are separate observations.
...
| GPUs | 2× AMD Radeon AI PRO R9700 |
| GPU target | `gfx1201` |
| PyTorch | `2.9.1+git8907517` |
| HIP runtime reported by PyTorch | `7.0.51831-a3e329ad8` |
| System ROCm | `7.1.x` |
| vLLM | `0.18.0+rocm700` |
| Model directory | `/home/manoj/models/qwen35a3b` |
...
![Original serving topology on shark](/images/qwen35a3b_r9700_system_diagram.svg)
...
| Stage | Recorded symptom | Interpretation and next check |
| --- | --- | --- |
| Restart | `10.11 / 29.86 GiB` free on each GPU; requested `29.26 GiB` | Check which processes still own device memory and recover the exact launch arguments. |
| Repeated or longer traffic | `FileNotFoundError: /vllm_tp_gather_1_to_0`, then `EngineDeadError` | Inspect the earliest error across ranks and the gather resource's lifetime. |
| After loading, before readiness | Weights and caches initialized; worker then exited | Locate the failing initialization stage in per-rank logs. |

![Original failure-path diagram](/images/qwen35a3b_r9700_failure_modes.svg)

The original restart diagnosis identified leftover workers. Stop the failed server's own processes and verify that memory is released. Do not kill unrelated workers or resource trackers on a shared machine.

## Result and follow-up

| Established by the recorded run | Still unresolved |
| --- | --- |
| The model loaded and served across both cards with CPU offload. | Repeatable startup and cleanup after failure. |
| A warm request returned 128 tokens at the reported 22.83 tok/s. | Sustained concurrency and long-context reliability. |
| Failures affected worker startup and interprocess coordination. | The initiating cause of each worker failure. |

For the next run, retain the exact command, checkpoint revision, request sequence, and timestamped rank logs. Report failed requests alongside successful latency samples. Change one setting at a time.
